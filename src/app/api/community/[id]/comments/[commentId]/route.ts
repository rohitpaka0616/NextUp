...
export async function PATCH(
  ...
  try {
    ...
    const { rows: ownerRows } = await pool.query(
      `SELECT id FROM "Comment" WHERE id = $1 AND "postId" = $2 AND "authorId" = $3`,
      [commentId, postId, session.user.id]
    );
    if (ownerRows.length === 0) return NextResponse.json({ error: "Not found" }, { status: 404 });

    const { rows } = await pool.query(`UPDATE "Comment" SET body = $1 WHERE id = $2 RETURNING *`, [body, commentId]);
