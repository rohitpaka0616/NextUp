...
export async function GET(req: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const { rows } = await pool.query(
      `SELECT c.id, c.body, c."authorId", c."ideaId", c."parentCommentId", c.upvotes, c."createdAt",
  ...
  try {
    ...
    if (parentCommentId) {
      const { rows: parentRows } = await pool.query(
        `SELECT id, "parentCommentId" FROM "Comment" WHERE id = $1 AND "ideaId" = $2`,
    ...
    const id = genId();
    const { rows } = await pool.query(
      `INSERT INTO "Comment" (id, body, "authorId", "ideaId", "parentCommentId", upvotes, "createdAt")
       VALUES ($1, $2, $3, $4, $5, 0, NOW())
       RETURNING *`,
      [id, text, session.user.id, ideaId, parentCommentId]
    );
    ...
    if (parentCommentId) {
      ...
      if (parentAuthorId && parentAuthorId !== session.user.id) {
        await pool.query(
          ...
          [genId(), parentAuthorId, `/ideas/${ideaId}#comment-${parentCommentId}`]
        );
      }
    } else {
      const { rows: ideaRows } = await pool.query(`SELECT "userId" FROM "Idea" WHERE id = $1 LIMIT 1`, [ideaId]);
      const ownerId = ideaRows[0]?.userId as string | undefined;
      if (ownerId && ownerId !== session.user.id) {
        await pool.query(
          `INSERT INTO "Notification" (id, "userId", type, "referenceId", read, "createdAt")
           VALUES ($1, $2, 'IDEA_COMMENT', $3, false, NOW())`,
          [genId(), ownerId, `/ideas/${ideaId}`]
        );
      }
    }
